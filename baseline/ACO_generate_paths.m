...
paths_collected = {};
for it = 1:max_iter
    fprintf("Iteração ACO %d/%d (α=%.2f, β=%.2f)\n", it, max_iter, alpha, beta);
    all_paths = cell(1,m);
    Ls = inf(1,m);
    for k = 1:m
        path = aco_single_walk(map_ref, start_pos, goal_pos, tau, alpha, beta);
        all_paths{k} = path;
        if ~isempty(path)
    ...
    % atualiza feromonio conforme (8)-(9) com penalidade sigma (10)
    delta = zeros(R,C);
    for k=1:m
        path = all_paths{k};
    ...
    % coletar caminhos que alcançaram objetivo (Ls finito)
    ...
    % opcional: limitamos growth para memória
    if length(paths_collected) > 5*m
        paths_collected = paths_collected(1:5*m);
    end
end

% garantir tamanho mínimo: replicar com mutações se necessário
...
d = length(maps_all);
% weights w_l (subjective probabilities) - we approximate from table if present
% If user needs exact w, we could pass them; here we use the default table values:
